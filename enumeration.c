...
sensor_catalog_entry_t sensor_catalog[] = {
        ...
        .channel =
            {
                ...
                {DECLARE_NAMED_CHANNEL("rot", "from_north_magnetic_tilt_comp")},
            },
    },
    {
        .tag = "intensity",
    ...
    },
    {
        ...
        .channel =
            {
                {
                    DECLARE_VOID_CHANNEL("steps").num_events = 1,
                    .event =
                        {
    ...
    },
    {
        ...
        .channel =
            {
                {
                    DECLARE_VOID_CHANNEL("proximity0").num_events = 1,
                    .event =
                        {
                ...
                {
                    DECLARE_VOID_CHANNEL("proximity1").num_events = 1,
                    .event =
                        {
                ...
                {
                    DECLARE_VOID_CHANNEL("proximity2").num_events = 1,
                    .event =
                        {
                ...
                {
                    DECLARE_VOID_CHANNEL("proximity3").num_events = 1,
                    .event =
                        {
...
static void setup_properties_from_pld(int s, int panel, int rotation, int num_channels) {
    /*
...
static int read_pld_from_properties(int s, int* panel, int* rotation) {
    ...
    if (sensor_get_prop(s, "panel", &p)) return -1;

    if (sensor_get_prop(s, "rotation", &r)) return -1;

    if (!is_valid_pld(p, r)) return -1;
...
static int read_pld_from_sysfs(int s, int dev_num, int* panel, int* rotation) {
    ...
    snprintf(sysfs_path, PATH_MAX, BASE_PATH "../firmware_node/pld/panel", dev_num);

    if (sysfs_read_int(sysfs_path, &p)) return -1;

    snprintf(sysfs_path, PATH_MAX, BASE_PATH "../firmware_node/pld/rotation", dev_num);

    if (sysfs_read_int(sysfs_path, &r)) return -1;
...
static int map_internal_to_external_type(int sensor_type) {
    /* Most sensors are internally identified using the Android type, but for some we use a
     * different type specification internally */
...
static void populate_descriptors(int s, int sensor_type) {
    ...
    ALOGV("Sensor %d (%s) type(%d) minD(%d) maxD(%ld) flags(%2.2lx)\n", s, sensor[s].friendly_name,
          sensor_desc[s].type, sensor_desc[s].minDelay, (long)sensor_desc[s].maxDelay,
          (long)sensor_desc[s].flags);
    ...
    sensor[s].max_supported_rate =
        min_delay_us && min_delay_us != -1 ? 1000000.0 / min_delay_us : 0;
}

static void release_plugin(int s) {
    int err;

    if (!sensor[s].plugin) return;

    if (sensor[s].plugin->close) {
        err = sensor[s].plugin->close(sensor[s].dev_num);
        if (err) ALOGE("Error while calling plugin close on sensor %d: %d\n", s, err);
        /* proceed further regardless the close error */
    }

    if (sensor[s].plugin->handle) dlclose(sensor[s].plugin->handle);

    free(sensor[s].plugin);
    sensor[s].plugin = NULL;
}

static void load_plugin(int s) {
    ...
    static const int paths_count = sizeof(paths) / sizeof(*paths);

    char plugin_name[MAX_NAME_SIZE];
    char plugin_path[PATH_MAX];
    void* handle;
    const char* dlsym_err;
    create_plugin_t create_plugin;
    int err, i;

    /* Load sensor plugin if specified */
    plugin_name[0] = '\0';
    ...
    if (!handle) {
        ALOGW("Plugin %s not found: %s\n", plugin_name, dlerror());
        return;
    }

    /* Reset errors */
    dlerror();
    create_plugin = (create_plugin_t)dlsym(handle, "create_plugin");
    dlsym_err = dlerror();
    if (dlsym_err) {
        ALOGE("Cannot find [create_plugin] function in %s: %s\n", plugin_name, dlsym_err);
        dlclose(handle);
        return;
    }

    if (create_plugin == NULL) {
        ALOGE("Failed at create plugin function %s.\n", plugin_name);
        dlclose(handle);
        return;
    }

    /* Instantiate the plugin */
    sensor[s].plugin = create_plugin();
    if (!sensor[s].plugin) {
        ALOGE("Failed to instantiate plugin %s.\n", plugin_name);
        dlclose(handle);
        return;
    }

    sensor[s].plugin->handle = handle;

    /* Invoke open() call */
    if (sensor[s].plugin->open) {
        err = sensor[s].plugin->open(sensor[s].dev_num);
        if (err) {
            ALOGE("Error while calling plugin open on sensor %d: %d\n", s, err);
            release_plugin(s);
        }
    }
}
...
static int add_sensor(int dev_num, int catalog_index, int mode) {
    ...
    if (sensor_type == SENSOR_TYPE_INTERNAL_ILLUMINANCE) {
        ...
        if (retval > 0) {
            snprintf(sysfs_path, PATH_MAX, ILLUMINATION_CALIBPATH, dev_num);
            sysfs_write_int(sysfs_path, retval);
        }
    }

    /*
     * See if we have optional calibration biases for each of the channels of this sensor. These
     * would be expressed using properties like iio.accel.y.calib_bias = -1, or possibly something
     * like iio.temp.calib_bias if the sensor has a single channel. This value gets stored in the
     * relevant calibbias sysfs file if that file can be located and then used internally by the iio
     * sensor driver.
     */
    ...
    if (num_channels) {
        for (c = 0; c < num_channels; c++) {
            ...
            snprintf(suffix, MAX_NAME_SIZE + 8, "%s.calib_bias", ch_name);
            if (!sensor_get_prop(s, suffix, &calib_bias) && calib_bias) {
                snprintf(suffix, MAX_NAME_SIZE + 8, "%s_%s", prefix,
                         sensor_catalog[catalog_index].channel[c].name);
                snprintf(sysfs_path, PATH_MAX, SENSOR_CALIB_BIAS_PATH, dev_num, suffix);
                sysfs_write_int(sysfs_path, calib_bias);
            }
        }
    } else if (!sensor_get_prop(s, "calib_bias", &calib_bias) && calib_bias) {
        snprintf(sysfs_path, PATH_MAX, SENSOR_CALIB_BIAS_PATH, dev_num, prefix);
        sysfs_write_int(sysfs_path, calib_bias);
    }
    ...
    if (mode == MODE_TRIGGER) {
        ...
        snprintf(sysfs_path, PATH_MAX, BUFFER_LENGTH_PATH, dev_num);
    ...
    for (c = 1; c < num_channels; c++) sensor[s].channel[c].opt_scale = 1;
    ...
    if (num_channels) {
        for (c = 0; c < num_channels; c++) {
            ch_name = sensor_catalog[catalog_index].channel[c].name;
            snprintf(suffix, MAX_NAME_SIZE + 8, "%s.opt_scale", ch_name);
    ...
    if (sensor_get_order(s, sensor[s].order)) sensor[s].quirks |= QUIRK_FIELD_ORDERING;
...
static void virtual_sensors_check(void) {
    ...
    for (i = 0; i < sensor_count; i++) switch (sensor[i].type) {
            case SENSOR_TYPE_ACCELEROMETER:
                has_acc = 1;
                break;
            case SENSOR_TYPE_GYROSCOPE:
    ...
    for (j = 0; j < catalog_size; j++) switch (sensor_catalog[j].type) {
...
static void propose_new_trigger(int s, char trigger_name[MAX_NAME_SIZE], int sensor_name_len) {
    /*
     * A new trigger has been enumerated for this sensor. Check if it makes sense to use it over the
     * currently selected one, and select it if it is so. The format is something like
     * sensor_name-dev0.
     */
...
static void update_sensor_matching_trigger_name(char name[MAX_NAME_SIZE], int* updated,
                                                int trigger) {
    /*
     * Check if we have a sensor matching the specified trigger name, which should then begin with
     * the sensor name, and end with a number equal to the iio device number the sensor is
     * associated to. If so, update the string we're going to write to trigger/current_trigger when
     * enabling this sensor.
     */
    ...
    /*
     * First determine the iio device number this trigger refers to. We expect the last few
     * characters (typically one) of the trigger name to be this number, so perform a few checks.
     */
...
static int create_hrtimer_trigger(int s, int trigger) {
    struct stat dir_status = {.st_dev = 0};
    ...
    snprintf(hrtimer_name, MAX_NAME_SIZE, "%s-hr-dev%d", sensor[s].internal_name,
             sensor[s].dev_num);
    snprintf(hrtimer_path, PATH_MAX, "%s%s", CONFIGFS_TRIGGER_PATH, buf);

    /* Get parent dir status */
    if (stat(CONFIGFS_TRIGGER_PATH, &dir_status) || !(dir_status.st_mode)) return -1;
...
static void setup_trigger_names(void) {
    ...
    for (s = 0; s < sensor_count; s++)
        snprintf(sensor[s].init_trigger_name, MAX_NAME_SIZE, "%s-dev%d", sensor[s].internal_name,
                 sensor[s].dev_num);
    ...
    /* If we don't have any other trigger exposed and quirk hrtimer is set setup the hrtimer name
     * here  - and create it also */
    ...
    /*
     * Certain drivers expose only motion triggers even though they should be continous. For these,
     * use the default trigger name as the motion trigger. The code generating intermediate events
     * is dependent on motion_trigger_name being set to a non empty string.
     */
...
static int catalog_index_from_sensor_type(int type) {
    ...
    for (i = 0; i < catalog_size; i++)
        if (sensor_catalog[i].type == type) return i;

    return -1;
}

static void post_process_sensor_list(char poll_map[catalog_size], char trig_map[catalog_size],
                                     char event_map[catalog_size]) {
    ...
    int illuminance_found = illuminance_cat_index >= 0 &&
                            (poll_map[illuminance_cat_index] || trig_map[illuminance_cat_index] ||
                             event_map[illuminance_cat_index]);

    /* If an illumimance sensor has been reported */
    if (illuminance_found && intensity_cat_index >= 0) {
...
static void reorder_sensors(void) {
    /* Some sensors may be marked as secondary - these need to be listed after other sensors of the
     * same type */
    int s1, s2, i;

    /* Mark temperature sensors that are not ambient sensors as secondary */
    for (i = 0; i < sensor_count; i++) {
        if ((sensor[i].type == SENSOR_TYPE_AMBIENT_TEMPERATURE) &&
            (strstr(sensor[i].friendly_name, "ambient") == NULL))
            sensor[i].quirks |= QUIRK_SECONDARY;
    }
...
void enumerate_sensors(void) {
    /*
     * Discover supported sensors and allocate control structures for them. Multiple sensors can
     * potentially rely on a single iio device (each using their own channels). We can't have
     * multiple sensors of the same type on the same device. In case of detection as both a
     * poll-mode and trigger-based sensor, use the trigger usage mode.
     */
    ...
    for (dev_num = 0; dev_num < MAX_DEVICES; dev_num++) {
        ...
        for (i = 0; i < catalog_size; i++) {
            ...
            if (event_sensors[i] && !add_sensor(dev_num, i, MODE_EVENT)) continue;
            ...
            if (poll_sensors[i]) add_sensor(dev_num, i, MODE_POLL);
        }

        if (trig_found) build_sensor_report_maps(dev_num);
    }
