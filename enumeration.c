...
static void populate_descriptors (int s, int sensor_type)
{
	...
	ALOGV("Sensor %d (%s) type(%d) minD(%d) maxD(%ld) flags(%2.2lx)\n",
		...
		sensor_desc[s].minDelay, (long)sensor_desc[s].maxDelay,
...
static void release_plugin(int s)
{
	...
	if (sensor[s].plugin->close) {
		...
		if (err)
			ALOGE("Error while calling plugin close on sensor %d: %d\n",
				s, err);
		/* proceed further regardless the close error */
	}

	if (sensor[s].plugin->handle)
		dlclose(sensor[s].plugin->handle);
...
static void load_plugin(int s)
{
	...
	plugin_name[0] = '\0';
	if (sensor_get_st_prop(s, "plugin", plugin_name) || !plugin_name[0])
		return;
	...
	if (sensor_type == SENSOR_TYPE_INTERNAL_ILLUMINANCE) {
		...
		for (c = 0; c < num_channels; c++) {
			...
			if (!sensor_get_prop(s, suffix, &calib_bias) && calib_bias) {
				snprintf(suffix, MAX_NAME_SIZE+8, "%s_%s", prefix, sensor_catalog[catalog_index].channel[c].name);
				 ...
				 int sensor_name_len)
{
	 ...
	 */
	strncpy_s(sensor[s].init_trigger_name, sizeof(sensor[s].init_trigger_name) - 1,
			trigger_name, MAX_NAME_SIZE);
}
...
static void update_sensor_matching_trigger_name (char name[MAX_NAME_SIZE], int* updated, int trigger)
{
	...
	for (s=0; s<sensor_count; s++)
		if (sensor[s].dev_num == dev_num) {
			...
			if ((sensor_name_len < MAX_NAME_SIZE - 1) && !strncmp(name, sensor[s].internal_name, sensor_name_len))
...
static int create_hrtimer_trigger(int s, int trigger)
{
	...
	if (stat(CONFIGFS_TRIGGER_PATH, &dir_status) || !(dir_status.st_mode))
		return -1;
...
static void post_process_sensor_list (char poll_map[catalog_size], char trig_map[catalog_size], char event_map[catalog_size])
{
	...
	int illuminance_found	  = illuminance_cat_index >= 0 && (poll_map[illuminance_cat_index] || trig_map[illuminance_cat_index] || event_map[illuminance_cat_index]);
