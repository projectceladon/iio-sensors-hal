...
#define PROP_BASE "ro.vendor.iio.%s.%s" /* Note: PROPERTY_KEY_MAX is small */
...
typedef struct {
    ...
    float opt_scale;                   /*
                                        * Optional correction scale read from a property such as
                                        * iio.accel.x.scale, allowing late compensation of problems
                                        * such as misconfigured axes ; set to 1 by default. Applied
...
typedef struct {
    ...
    /*
     * Currently active trigger - either a pointer to the initial (default) trigger name buffer, or
     * a pointer to the motion trigger name buffer, or something else (typically NULL or a pointer
     * to some static "\n". This is used to determine if the conditions are met to switch from the
     * default trigger to the motion trigger for a sensor, or rather for the interrupt-driven
     * sensors associated to a given iio device.
     */
    ...
    /*
     * The array below indicates where to gather report data for this sensor inside the reports that
     * we read from the iio character device. It is updated whenever channels are enabled or
     * disabled on the same device. Channel size indicates the size in bytes of fields, and should
     * be zero for disabled channels. The type field indicates how a specific channel data item is
     * structured.
     */
    channel_info_t channel[MAX_CHANNELS];

    /*
     * This flag is set if we acquired data from the sensor but did not forward it to upper layers
     * (i.e. Android) yet. If so, report_buffer contains that data. Valid values are 0: empty, 1:
     * normal, 2: repeat of last acquired value after timeout.
     */
    int report_pending;
    ...
    /*
     * Timestamp closely matching the date of sampling, preferably retrieved from a iio channel
     * alongside sample data. Value zero indicates that we couldn't get such a closely correlated
     * timestamp, and that one has to be generated before the report gets sent up to Android.
     */
    int64_t report_ts;
    ...
    /*
     * Depending on the sensor, calibration may take too much time at higher levels. Allow optional
     * capping to a certain level.
     */
    int max_cal_level;
    ...
    /*
     * Certain sensors expose their readings through sysfs files that have a long response time
     * (100-200 ms for ALS). Rather than block our global control loop for several hundred ms each
     * second, offload those lengthy blocking reads to dedicated threads, which will then report
    ...
    uint32_t ref_count; /* Dependency count - for a real sensor how many active virtual sensors are
                           depending on it */

    uint32_t directly_enabled; /* Flag showing if a sensor was enabled directly by Android */

    /*
     * Current sample for a virtual sensor - when a report is ready we'll keep the data here until
     * it's finally processed. Can be modified for more than one at a later time.
     */
    sensors_event_t sample;
    uint64_t event_id;

    /*
     * If the QUIRK_FIELD_ORDERING bit is set in quirks, the contents of this array are used in the
     * finalization stage to swap sample fields before transmitting them to Android ; they form a
     * mapping between the indices of the input and output arrays: ex: 0123 is identity for a sample
     * containing 4 fields.
     */
    unsigned char order[MAX_CHANNELS];

    /*
     * If the QUIRK_MOUNTING_MATRIX bit is set in quirks, the contents of this matrix is used to
     * correct the sample values so that it takes into account the way the sensor has been mounted
     * on the PCB.
     */
    ...
    /*
     * Event counter - will be used to check if we have a significant sample for noisy sensors. We
     * want to make sure we do not send any wrong events before filtering kicks in. We can also use
     * it for statistics.
     */
    ...
    float semi_arbitrated_rate; /* Arbitrated sampling rate before we considered other sensors
                                   co-located on the same iio device */

    /* Sensor plugin */
    sensor_plugin_t *plugin;
} sensor_info_t;
...
#define DECLARE_VOID_CHANNEL(tag) tag, "", "", "", "", "", "",

#define DECLARE_CHANNEL(tag, spacer, name)                            \
    name, "in_" tag spacer name "_en", "in_" tag spacer name "_type", \
        "in_" tag spacer name "_index", "in_" tag spacer name "_raw", \
        "in_" tag spacer name "_input", "in_" tag spacer name "_scale", 0, {{0}},
...
#define DECLARE_EVENT(tag, spacer1, name, spacer2, type, spacer3, dir) \
    type, dir, "in_" tag spacer1 name spacer2 type spacer3 dir "_en",  \
...
#define DECLARE_NAMED_EVENT(tag, name) DECLARE_EVENT(tag, "_", name, "", "", "", "")
