...
static int events_fd[MAX_DEVICES]; /* fd on the /sys/bus/iio/devices/iio:deviceX/events/<event_name>
                                      file */
 ...
 * - THREAD_REPORT_TAG_BASE + sensor handle if the fd is the receiving end of a pipe used by a sysfs
...
inline int is_enabled(int s) { return sensor[s].directly_enabled || sensor[s].ref_count; }
...
static int check_state_change(int s, int enabled, int from_virtual) {
    if (enabled) {
        ...
        if (!from_virtual) sensor[s].directly_enabled = 1; /* We're being directly enabled */

        if (sensor[s].ref_count) return 0; /* We were already indirectly enabled */

        return 1; /* Do continue enabling this sensor */
    }

    if (!is_enabled(s)) return 0; /* We are being disabled but already were: no change */
    ...
    if (!from_virtual && sensor[s].ref_count) return 0; /* We still have ref counts */
...
static int enable_buffer(int dev_num, int enabled) {
    ...
    snprintf(sysfs_path, sizeof(sysfs_path), ENABLE_PATH, dev_num);

    while (retries) {
        /* Low level, non-multiplexed, enable/disable routine */
        if (sysfs_write_int(sysfs_path, enabled) > 0) return 0;
...
static int setup_trigger(int s, const char *trigger_val) {
    ...
    snprintf(sysfs_path, sizeof(sysfs_path), TRIGGER_PATH, sensor[s].dev_num);
...
static int enable_event(int dev_num, const char *name, int enabled) {
    ...
    snprintf(sysfs_path, PATH_MAX, EVENTS_PATH "%s", dev_num, name);
...
static int enable_sensor(int dev_num, const char *tag, int enabled) {
    ...
    snprintf(sysfs_path, PATH_MAX, SENSOR_ENABLE_PATH, dev_num, tag);
    return sysfs_write_int(sysfs_path, enabled);
}
...
static void enable_iio_timestamp(int dev_num, int known_channels) {
    ...
    snprintf(sysfs_path, PATH_MAX, CHANNEL_PATH "%s", dev_num, "in_timestamp_type");

    n = sysfs_read_str(sysfs_path, spec_buf, sizeof(spec_buf));

    if (n <= 0) return;

    if (strcmp(spec_buf, "le:s64/64>>0")) return;

    /* OK, type is int64_t as expected, in little endian representation */

    snprintf(sysfs_path, PATH_MAX, CHANNEL_PATH "%s", dev_num, "in_timestamp_index");

    if (sysfs_read_int(sysfs_path, &n)) return;

    /* Check that the timestamp comes after the other fields we read */
    if (n != known_channels) return;

    /* Try enabling that channel */
    snprintf(sysfs_path, PATH_MAX, CHANNEL_PATH "%s", dev_num, "in_timestamp_en");
...
static int decode_type_spec(const char type_buf[MAX_TYPE_SPEC_LEN], datum_info_t *type_info) {
    ...
    tokens =
        sscanf(type_buf, "%ce:%c%u/%u>>%u", &endianness, &sign, &realbits, &storagebits, &shift);
...
void build_sensor_report_maps(int dev_num) {
    /*
     * Read sysfs files from a iio device's scan_element directory, and build a couple of tables
     * from that data. These tables will tell, for each sensor, where to gather relevant data in a
     * device report, i.e. the structure that we read from the /dev/iio:deviceX file in order to
     * sensor report, itself being the data that we return to Android when a sensor poll completes.
     * The mapping should be straightforward in the case where we have a single sensor active per
     * iio device but, this is not the general case. In general several sensors can be handled
     * through a single iio device, and the _en, _index and _type syfs entries all concur to paint a
     * picture of what the structure of the device report is.
     */
    ...
    for (s = 0; s < sensor_count; s++) {
        if (sensor[s].dev_num != dev_num) continue;
        ...
        for (c = 0; c < sensor[s].num_channels; c++) {
            /* Read _type file */
            snprintf(sysfs_path, PATH_MAX, CHANNEL_PATH "%s", sensor[s].dev_num,
    ...
    /*
     * Now that we know which channels are defined, their sizes and their ordering, update channels
     * offsets within device report. Note: there is a possibility that several sensors share the
     * same index, with their data fields being isolated by masking and shifting as specified
     * through the real bits and shift values in type attributes. This case is not currently
     * supported. Also, the code below assumes no hole in the sequence of indices, so it is
     * dependent on discovery of all sensors.
     */
    offset = 0;
    ...
    if (has_iio_ts[dev_num]) offset = (offset + 7) / 8 * 8 + sizeof(int64_t);
    ...
    if (expected_dev_report_size[dev_num] > MAX_DEVICE_REPORT_SIZE) {
        ALOGE("Unexpectedly large scan buffer on iio dev%d: %d bytes\n", dev_num,
              expected_dev_report_size[dev_num]);
...
void set_report_ts(int s, int64_t ts) {
    ...
    if (sensor[s].report_ts && sensor[s].sampling_rate &&
        ...
        if (ts - maxTs >= MAX_DELAY) maxTs = ts;
...
static void *acquisition_routine(void *param) {
    /*
     * Data acquisition routine run in a dedicated thread, covering a single sensor. This loop will
     * periodically retrieve sampling data through sysfs, then package it as a sample and transfer
     * it to our master poll loop through a report fd. Checks for a cancellation signal quite
     * frequently, as the thread may be disposed of at any time. Note that Bionic does not provide
    ...
    /*
     * Each condition variable is associated to a mutex that has to be locked by the thread that's
    ...
    while (sensor[s].thread_data_fd[1] != -1) {
        ...
        for (c = 0; c < num_fields; c++) {
            ...
            /* Check and honor termination requests */
            if (sensor[s].thread_data_fd[1] == -1) goto exit;
        }
        ...
        if (sensor[s].ops.finalize(s, &data)) {
            ...
            if (ret != sizeof(sensors_event_t))
                ALOGE("S%d write failure: wrote %zu, got %d\n", s, sizeof(sensors_event_t), ret);
        }

        /* Check and honor termination requests */
        if (sensor[s].thread_data_fd[1] == -1) goto exit;

        /* Recalculate period assuming sensor[s].sampling_rate can be changed dynamically during the
         * thread run */
        if (sensor[s].sampling_rate <= 0) {
        ...
        /* Wait until the sampling time elapses, or a rate change is signaled, or a thread exit is
         * requested */
        ret =
            pthread_cond_timedwait(&thread_release_cond[s], &thread_release_mutex[s], &target_time);
    }
...
static int is_fast_accelerometer(int s) {
    /*
     * Some games don't react well to accelerometers using any-motion triggers. Even very low
     * thresholds seem to trip them, and they tend to request fairly high event rates. Favor
     * continuous triggers if the sensor is an accelerometer and uses a sampling rate of at
     * least 25.
     */

    if (sensor[s].type != SENSOR_TYPE_ACCELEROMETER) return 0;

    if (sensor[s].sampling_rate < 25) return 0;
...
static void tentative_switch_trigger(int s) {
     ...
     *
     * - for applications using the accelerometer with high sampling rates, prefer the continuous
     * trigger over the any-motion one, to avoid jumps related to motion thresholds
     */

    if (is_fast_accelerometer(s) && !(sensor[s].quirks & QUIRK_TERSE_DRIVER) &&
...
static float get_group_max_sampling_rate(int s) {
    ...
    if (is_enabled(s)) arbitrated_rate = sensor[s].requested_rate;

    /* If any of the currently active sensors built on top of this one need a higher sampling rate,
     * switch to this rate */
    for (i = 0; i < sensor_count; i++)
        for (vi = 0; vi < sensor[i].base_count; vi++)
            if (sensor[i].base[vi] == s && is_enabled(i) &&
                sensor[i].requested_rate > arbitrated_rate) /* If sensor i depends on sensor s */
                arbitrated_rate = sensor[i].requested_rate;

    /* If any of the currently active sensors we rely on is using a higher sampling rate, switch to
     * this rate */
...
static float select_closest_available_rate(int s, float requested_rate) {
    ...
    if (!sensor[s].avail_freqs_count) return requested_rate;

    for (j = 0; j < sensor[s].avail_freqs_count; j++) {
        sr = sensor[s].avail_freqs[j];

        /* If this matches the selected rate, we're happy.  Have some tolerance for rounding errors
         * and avoid needless jumps to higher rates */
        ...
        /*
         * If we reached a higher value than the desired rate, adjust selected rate so it matches
         * the first higher available one and stop parsing - this makes the assumption that rates
         * are sorted by increasing value in the allowed frequencies string.
         */
...
static int sensor_set_rate(int s, float requested_rate) {
    /* Set the rate at which a specific sensor should report events. See Android sensors.h for
     * indication on sensor trigger modes */
    ...
    if (arb_sampling_rate < sensor[s].min_supported_rate) {
        ALOGV("Sampling rate %g too low for %s, using %g instead\n", arb_sampling_rate,
              sensor[s].friendly_name, sensor[s].min_supported_rate);
    ...
    if (sensor[s].max_supported_rate && arb_sampling_rate > sensor[s].max_supported_rate) {
        ALOGV("Sampling rate %g too high for %s, using %g instead\n", arb_sampling_rate,
              sensor[s].friendly_name, sensor[s].max_supported_rate);
    ...
    if (sensor[s].is_virtual) return 0;

    /* If we're dealing with a poll-mode sensor */
    if (sensor[s].mode == MODE_POLL) {
        if (is_enabled(s))
            pthread_cond_signal(
                &thread_release_cond[s]); /* Wake up thread so the new sampling rate gets used */
        return 0;
    }

    snprintf(sysfs_path, PATH_MAX, SENSOR_SAMPLING_PATH, dev_num, prefix);
    ...
    if (sysfs_read_float(sysfs_path, &cur_sampling_rate) != -1) {
        ...
        snprintf(sysfs_path, sizeof(sysfs_path), DEVICE_SAMPLING_PATH, dev_num);
    ...
    /* Record the rate that was agreed upon with the sensor taken in isolation ; this avoid
     * uncontrolled ripple effects between colocated sensor rates */
    ...
    /* Update actual sampling rate field for this sensor and others which may be sharing the same
     * sampling rate */
    if (per_device_sampling_rate)
        for (n = 0; n < sensor_count; n++)
    ...
    if (arb_sampling_rate == cur_sampling_rate) return 0;
    ...
    if (trig_sensors_per_dev[dev_num]) enable_buffer(dev_num, 0);
    ...
    if (trig_sensors_per_dev[dev_num]) enable_buffer(dev_num, 1);
...
static void reapply_sampling_rates(int s) {
    /*
     * The specified sensor was either enabled or disabled. Other sensors in the same group may have
     * constraints related to this sensor sampling rate on their own sampling rate, so reevaluate
     * them by retrying to use their requested sampling rate, rather than the one that ended up
     * being used after arbitration.
     */
...
int sensor_activate(int s, int enabled, int from_virtual) {
    ...
    if (sensor[s].is_virtual) return sensor_activate_virtual(s, enabled, from_virtual);
    ...
    if (enabled) setup_noise_filtering(s); /* Initialize filtering data if required */
    ...
    /*
     * Make sure we have a fd on the character device ; conversely, close the fd if no one is using
     * associated sensors anymore. The assumption here is that the underlying driver will power on
     * the relevant hardware block while someone holds a fd on the device.
     */
    dev_fd = device_fd[dev_num];

    if (!enabled) {
        if (sensor[s].mode == MODE_POLL) stop_acquisition_thread(s);

        if (dev_fd != -1 && !poll_sensors_per_dev[dev_num] && !trig_sensors_per_dev[dev_num]) {
            /* Stop watching this fd. This should be a no-op in case this fd was not in the poll
             * set. */
        ...
        if (sensor[s].mode == MODE_EVENT) {
            ...
            for (c = 0; c < sensor_catalog[catalog_index].num_channels; c++) {
                for (d = 0; d < sensor_catalog[catalog_index].channel[c].num_events; d++)
                    enable_event(dev_num,
                                 sensor_catalog[catalog_index].channel[c].event[d].ev_en_path,
                                 enabled);
            }
    ...
    if (dev_fd == -1) {
        ...
        snprintf(device_name, sizeof(device_name), DEV_FILE_PATH, dev_num);
    ...
    if (sensor[s].mode == MODE_POLL) start_acquisition_thread(s);
...
static void enable_motion_trigger(int dev_num) {
    /*
     * In the ideal case, we enumerate two triggers per iio device ; the default (periodically
     * firing) trigger, and another one (the motion trigger) that only fires up when motion is
     * detected. This second one allows for lesser energy consumption, but requires periodic sample
     * duplication at the HAL level for sensors that Android defines as continuous. This "duplicate
     * last sample" logic can only be engaged once we got a first sample for the driver, so we start
     * with the default trigger when an iio device is first opened, then adjust the trigger when we
     * got events for all active sensors. Unfortunately in the general case several sensors can be
     * associated to a given iio device, they can independently be controlled, and we have to adjust
     * the trigger in use at the iio device level depending on whether or not appropriate conditions
     * are met at the sensor level.
     */
    ...
    if (!active_sensors) return;

    /* Check that all active sensors are ready to switch */

    for (s = 0; s < MAX_SENSORS; s++)
        if (sensor[s].dev_num == dev_num && is_enabled(s) && sensor[s].num_channels &&
            (!sensor[s].motion_trigger_name[0] || !sensor[s].report_initialized ||
             is_fast_accelerometer(s) || (sensor[s].quirks & QUIRK_FORCE_CONTINUOUS)))
            return; /* Nope */
...
static int integrate_device_report_from_dev(int dev_num, int fd) {
    ...
    if (len == expected_dev_report_size[dev_num]) ts = *(int64_t *)(buf + ts_offset);
    ...
    ALOGV("Driver timestamp on iio device %d: ts=%ld\n", dev_num, (long)ts);
...
static int integrate_device_report_from_event(int dev_num, int fd) {
    ...
    ALOGV("Read event %lld from fd %d of iio device %d - ts %ld\n", event.id, fd, dev_num,
          (long)ts);
...
static int propagate_sensor_report(int s, sensors_event_t *data) {
    ...
    /* The finalize routine, in addition to its late sample processing duty, has the final say on
     * whether or not the sample gets sent to Android */
    return ret;
}

static void synthetize_duplicate_samples(void) {
    /*
     * Some sensor types (ex: gyroscope) are defined as continuously firing by Android, despite the
     * fact that we can be dealing with iio drivers that only report events for new samples. For
     * these we generate reports periodically, duplicating the last data we got from the driver.
     * This is not necessary for polling sensors.
     */
...
static void integrate_thread_report(uint32_t tag) {
    ...
    if (len == sizeof(sensors_event_t)) sensor[s].report_pending = DATA_SYSFS;
}
...
static int get_poll_wait_timeout(void) {
    ...
    /*
     * Check if we're dealing with a driver that only send events when there is motion, despite the
     * fact that the associated Android sensor type is continuous rather than on-change. In that
     * case we have to duplicate events. Check deadline for the nearest upcoming event.
     */
    for (s = 0; s < sensor_count; s++)
        if (is_enabled(s) && sensor[s].selected_trigger == sensor[s].motion_trigger_name &&
            sensor[s].sampling_rate) {
            period = (int64_t)(1000000000.0 / sensor[s].sampling_rate);

            if (sensor[s].report_ts + period < target_ts) target_ts = sensor[s].report_ts + period;
        }

    /* If we don't have such a driver to deal with */
    if (target_ts == INT64_MAX) return -1; /* Infinite wait */

    ms_to_wait = (target_ts - get_timestamp_boot()) / 1000000;

    /* If the target timestamp is already behind us, don't wait */
    if (ms_to_wait < 1) return 0;
...
return_available_sensor_reports:
    ...
    for (s = 0; s < sensor_count && returned_events < count; s++) {
        if (sensor[s].report_pending) {
            ...
            /*
             * If the sample was deemed invalid or unreportable, e.g. had the same value as the
             * previously reported value for a 'on change' sensor, silently drop it.
             */
        }
    ...
    if (returned_events) return returned_events;
...
await_event:
    ...
    for (i = 0; i < nfds; i++)
        if (ev[i].events == EPOLLIN) switch (ev[i].data.u32) {
                case 0 ... MAX_DEVICES - 1:
                    /* Read report from iio char dev fd */
                    integrate_device_report(ev[i].data.u32);
                    break;

                case THREAD_REPORT_TAG_BASE ... THREAD_REPORT_TAG_BASE + MAX_SENSORS - 1:
...
int sensor_set_delay(int s, int64_t ns) {
    ...
    if (ns <= 0) {
        ALOGE("Invalid delay requested on sensor %d: %ld\n", s, (long)ns);
        return -EINVAL;
    }
    ...
    ALOGV("Entering set delay S%d (%s): current rate: %g, requested: %g\n", s,
          sensor[s].friendly_name, sensor[s].sampling_rate, requested_sampling_rate);

    /*
     * Only try to adjust the low level sampling rate if it's different from the current one, as set
     * by the HAL. This saves a few sysfs reads and writes as well as buffer enable/disable
     * operations, since at the iio level most drivers require the buffer to be turned off in order
     * to accept a sampling rate change. Of course that implies that this field has to be kept up to
     * date and that only this library is changing the sampling rate.
     */
...
int sensor_flush(int s) {
    ...
    if (sensor_desc[s].flags & SENSOR_FLAG_ONE_SHOT_MODE || !is_enabled(s)) return -EINVAL;
