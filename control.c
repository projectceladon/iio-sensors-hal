...
void build_sensor_report_maps (int dev_num)
{
	...
	for (s=0; s<sensor_count; s++) {
		...
		for (c=0; c<sensor[s].num_channels; c++) {
			...
			/* Read _type file */
			snprintf(sysfs_path, PATH_MAX, CHANNEL_PATH "%s", sensor[s].dev_num, sensor_catalog[i].channel[c].type_path);
			...
			/* Read _index file */
			snprintf(sysfs_path, PATH_MAX, CHANNEL_PATH "%s", sensor[s].dev_num, sensor_catalog[i].channel[c].index_path);
		...
		for (c=0;c<sensor[s].num_channels; c++) {
			snprintf(sysfs_path, PATH_MAX, CHANNEL_PATH "%s", sensor[s].dev_num, sensor_catalog[i].channel[c].en_path);
			sysfs_write_int(sysfs_path, 1);
		}
	}
