...
/*
 * This implements a crude way of estimating the accelerometer manufacturing mounting bias. We
 * monitor x y and z distribution around a few strategic spots that should represent most hits when
 * the device is stable (on earth!). We then try to derive an estimation of the accelerometer bias
 * for each of these axes from that data, continuously. This is a very rough method that should only
 * be used as a last resort for now.
 */
...
#define ACCEL_CALIBRATION_PATH \
    "/data/vendor/sensors/accel.conf" /* Location of saved calibration data */
...
static float estimate_bias(accel_cal_t* cal_data, int channel) {
    /*
     * The long term distribution within the bucket, for each of the buckets, should be centered
     * (samples evenly distributed). Try to determine the position in the bucket that separates it
     * in two portions holding as many samples, then compute an estimated bias for that axis
    ...
    uint64_t bias_weight[BUCKET_COUNT] = {0};
    ...
    for (i = 0; i < BUCKET_COUNT; i++) total_weight += bias_weight[i];
    ...
    for (i = 0; i < BUCKET_COUNT; i++)
        if (bias_weight[i])
            estimated_bias +=
                estimated_bucket_bias[i] * (float)bias_weight[i] / (float)total_weight;
...
void accel_cal_init(int s) {
    ...
    if (fd != -1) {
        ...
        if (n == sizeof(accel_cal_t) &&
            ...
            cal_data->bucket_count == BUCKET_COUNT && cal_data->slices == SLICES &&
    ...
    /* Store the parameters that are used with that data set, so we can check them against future
     * version of the code to prevent inadvertent reuse */
    cal_data->version = (ACCEL_CALIB_DATA_VERSION << 16) + sizeof(accel_cal_t);
