...
#define SLICES                                                                 \
    100 /* Currently have 3 buckets per axis, and 100 slices per bucket ; then \
           we distribute incoming samples among slices */
...
typedef struct {
    ...
    uint64_t bucket[3][BUCKET_COUNT][SLICES]; /* How many samples fell in each of the slices, \
                                                 per axis */
    uint64_t bucket_usage[3][BUCKET_COUNT];   /* How many samples fell in each of the buckets (= sum
                                                 of the slice counts) */
