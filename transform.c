...
inline unsigned int set_bit_range(int start, int end) {
    ...
    for (i = start; i < end; ++i) value |= BIT(i);
...
inline float convert_from_vtf_format(int size, int exponent, unsigned int value) {
    ...
    if (exponent < 0) {
        ...
        for (i = 0; i < exponent; ++i) divider = divider * 10;
...
#define CONVERT_A_G_VTF16E14_X(s, d, x) \
    (DEF_ORIENT_ACCEL_X * convert_from_vtf_format(s, d, x) * GRAVITY)
#define CONVERT_A_G_VTF16E14_Y(s, d, x) \
    (DEF_ORIENT_ACCEL_Y * convert_from_vtf_format(s, d, x) * GRAVITY)
#define CONVERT_A_G_VTF16E14_Z(s, d, x) \
    (DEF_ORIENT_ACCEL_Z * convert_from_vtf_format(s, d, x) * GRAVITY)

/* Degree/sec to radian/sec */
#define CONVERT_G_D_VTF16E14_X(s, d, x) \
    (DEF_ORIENT_GYRO_X * convert_from_vtf_format(s, d, x) * M_PI / 180)
#define CONVERT_G_D_VTF16E14_Y(s, d, x) \
    (DEF_ORIENT_GYRO_Y * convert_from_vtf_format(s, d, x) * M_PI / 180)
#define CONVERT_G_D_VTF16E14_Z(s, d, x) \
    (DEF_ORIENT_GYRO_Z * convert_from_vtf_format(s, d, x) * M_PI / 180)
...
static int64_t sample_as_int64(unsigned char* sample, datum_info_t* type) {
    ...
    if (type->endianness == 'b')
        for (i = 0; i < type->storagebits / 8; i++) u64 = (u64 << 8) | sample[i];
    else
        for (i = type->storagebits / 8 - 1; i >= 0; i--) u64 = (u64 << 8) | sample[i];

    u64 = (u64 >> type->shift) & (~0ULL >> zeroed_bits);

    if (type->sign == 'u') return (int64_t)u64; /* We don't handle unsigned 64 bits int */
...
static void reorder_fields(float* data, unsigned char map[MAX_CHANNELS]) {
    ...
    for (i = 0; i < MAX_CHANNELS; i++) temp[i] = data[map[i]];

    for (i = 0; i < MAX_CHANNELS; i++) data[i] = temp[i];
}
...
static void clamp_gyro_readings_to_zero(int s, sensors_event_t* data) {
    ...
    if (fabs(x) < near_zero && fabs(y) < near_zero && fabs(z) < near_zero) {
         ...
         * (orientation, rotation vector) don't seem to react very well to it.
         */
...
static void process_event(int s, sensors_event_t* data) {
    /*
     * This gets the real event (post process - calibration, filtering & co.) and makes it into a
     * virtual one. The specific processing function for each sensor will populate the necessary
     * fields and set up the report pending flag.
     */
...
static int plugin_process(int s, sensors_event_t* data, int preprocessing) {
    int ret;
    int (*process)(sensors_event_t * evt);

    if (!sensor[s].plugin) return 0; /* Pass the sample to Android */

    process = preprocessing ? sensor[s].plugin->preprocess : sensor[s].plugin->postprocess;
    if (process) {
        ret = process(data);
        if (ret) {
            ALOGE("Error while %sprocessing data by sensor S%d: %d\n",
                  preprocessing ? "pre" : "post", s, ret);
            return ret; /* Drop the sample */
        }
    }

    return 0; /* Pass the sample to Android */
}

#define preprocess(s, data) plugin_process(s, data, 1)
#define postprocess(s, data) plugin_process(s, data, 0)
...
static int finalize_sample_default(int s, sensors_event_t* data) {
    ...
    switch (sensor[s].type) {
        ...
        case SENSOR_TYPE_GYROSCOPE:
            ...
            /* Report medium accuracy by default ; higher accuracy levels will be reported once, and
             * if, we achieve  calibration. */
            data->gyro.status = SENSOR_STATUS_ACCURACY_MEDIUM;

            /*
             * We're only trying to calibrate data from continuously firing gyroscope drivers, as
             * motion based ones use movement thresholds that may lead us to incorrectly estimate
             * bias.
             */
            ...
            /*
             * For noisy sensors drop a few samples to make sure we have at least GYRO_MIN_SAMPLES
             * events in the filtering queue. This improves mean and std dev.
             */
            if (sensor[s].filter_type) {
                if (sensor[s].selected_trigger != sensor[s].motion_trigger_name &&
                    sensor[s].event_count < GYRO_MIN_SAMPLES) {
                    postprocess(s, data);
                    /* drop the sample regardless the error code returned by postprocess */
                    return 0;
                }
        ...
        case SENSOR_TYPE_PROXIMITY:
             ...
             */
            if (data->data[0] > PROXIMITY_THRESHOLD) data->data[0] = PROXIMITY_THRESHOLD;

            /* ... fall through ... */
        case SENSOR_TYPE_LIGHT:
        case SENSOR_TYPE_AMBIENT_TEMPERATURE:
            /* Thermal values are multipled by 1000. So, actual value is (1/1000)th of it */
            data->data[0] = 0.001 * (int)(data->data[0]);

            /* These are on change sensors ; drop the sample if it has the same value as the
...
void select_transform(int s) {
    ...
    snprintf(prop_name, PROP_NAME_MAX, PROP_BASE, prefix, "transform");
...
float acquire_immediate_float_value(int s, int c) {
    ...
    if (sensor[s].channel[c].input_path_present) {
        snprintf(sysfs_path, PATH_MAX, BASE_PATH "%s", dev_num, input_path);
    ...
    if (!sensor[s].channel[c].raw_path_present) return 0;

    snprintf(sysfs_path, PATH_MAX, BASE_PATH "%s", dev_num, raw_path);
    ret = sysfs_read_float(sysfs_path, &val);
