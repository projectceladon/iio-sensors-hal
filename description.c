...
#define MIN_ON_CHANGE_SAMPLING_PERIOD_US                                                           \
    200000 /* For on change sensors (temperature, proximity, ALS, etc.) report we support 5 Hz max \
              (0.2 s min period) */
#define MAX_ON_CHANGE_SAMPLING_PERIOD_US 10000000 /* 0.1 Hz min (10 s max period)*/
#define ANDROID_MAX_FREQ \
    1000 /* 1000 Hz - This is how much Android requests for the fastest frequency */
...
int sensor_get_st_prop(int s, const char* sel, char val[MAX_NAME_SIZE]) {
    ...
    snprintf(extended_sel, PROP_NAME_MAX, "%s.%s", sensor[s].internal_name, sel);
...
int sensor_get_prop(int s, const char* sel, int* val) {
    ...
    if (sensor_get_st_prop(s, sel, buf)) return -1;
...
char* sensor_get_name(int s) {
    ...
    if (sensor[s].internal_name[0]) {
        snprintf(sensor[s].friendly_name, MAX_NAME_SIZE, "S%d-%s", s, sensor[s].internal_name);
    } else {
...
char* sensor_get_vendor(int s) {
    ...
    if (sensor[s].vendor_name[0] || !sensor_get_st_prop(s, "vendor", sensor[s].vendor_name))
        return sensor[s].vendor_name;

    return "";
}

int sensor_get_version(__attribute__((unused)) int s) { return IIO_SENSOR_HAL_VERSION; }

void sensor_update_max_range(int s) {
    if (sensor[s].max_range) return;
    ...
    if (sensor[s].num_channels && sensor[s].channel[0].type_info.realbits) {
        switch (sensor[s].type) {
            ...
            default:
                sensor[s].max_range =
                    (1ULL << sensor[s].channel[0].type_info.realbits) * sensor[s].resolution +
                    (sensor[s].offset || sensor[s].channel[0].offset);
                break;
        }
    }
    ...
    if (sensor[s].max_range) sensor_desc[s].maxRange = sensor[s].max_range;
}
...
float sensor_get_max_range(int s) {
    ...
    if (sensor[s].max_range != 0.0 || !sensor_get_fl_prop(s, "max_range", &sensor[s].max_range))
...
float sensor_get_min_freq(int s) {
    ...
    if (!sensor_get_fl_prop(s, "min_freq", &min_freq)) return min_freq;
...
float sensor_get_max_freq(int s) {
    ...
    if (!sensor_get_fl_prop(s, "max_freq", &max_freq)) return max_freq;
...
int sensor_get_cal_steps(int s) {
    int cal_steps;
    if (!sensor_get_prop(s, "cal_steps", &cal_steps)) return cal_steps;
...
float sensor_get_power(int s) {
    ...
    if (sensor[s].power != 0.0 || !sensor_get_fl_prop(s, "power", &sensor[s].power))
...
uint32_t sensor_get_quirks(int s) {
    ...
    if (!(sensor[s].quirks & QUIRK_ALREADY_DECODED)) {
        ...
        if (strstr(quirks_buf, "init-rate")) sensor[s].quirks |= QUIRK_INITIAL_RATE;

        if (strstr(quirks_buf, "continuous")) sensor[s].quirks |= QUIRK_FORCE_CONTINUOUS;

        if (strstr(quirks_buf, "terse")) sensor[s].quirks |= QUIRK_TERSE_DRIVER;

        if (strstr(quirks_buf, "noisy")) sensor[s].quirks |= QUIRK_NOISY;

        if (strstr(quirks_buf, "biased")) sensor[s].quirks |= QUIRK_BIASED;

        if (strstr(quirks_buf, "spotty")) sensor[s].quirks |= QUIRK_SPOTTY;

        if (strstr(quirks_buf, "no-event")) sensor[s].quirks |= QUIRK_NO_EVENT_MODE;

        if (strstr(quirks_buf, "no-trig")) sensor[s].quirks |= QUIRK_NO_TRIG_MODE;

        if (strstr(quirks_buf, "no-poll")) sensor[s].quirks |= QUIRK_NO_POLL_MODE;

        if (strstr(quirks_buf, "hrtimer")) sensor[s].quirks |= QUIRK_HRTIMER;

        if (strstr(quirks_buf, "secondary")) sensor[s].quirks |= QUIRK_SECONDARY;
...
int sensor_get_order(int s, unsigned char map[MAX_CHANNELS]) {
    ...
    if (sensor_get_st_prop(s, "order", buf)) return 0; /* No order property */
    ...
    for (i = 0; i < count; i++) map[i] = buf[i] - '0';
...
int sensor_get_mounting_matrix(int s, float mm[9]) {
    ...
    for (i = 0; i < 9; i++) {
        ...
        f = strtof(tmp1, &tmp2);
        if (!f && tmp1 == tmp2) return 0;
        mm[i] = f;
        tmp1 = tmp2 + 1;
    }

    /*
     * For proximity sensors, interpret a negative final z value as a hint that the sensor is back
     * mounted. In that case, mark the sensor as secondary to ensure that it gets listed after other
     * sensors of same type that would be front-mounted. Most applications will only ask for the
     * default proximity sensor and it makes more sense to point to, say, the IR based proximity
     * sensor rather than SAR based one if we have both, as on SoFIA LTE MRD boards.
     */
    ...
    ALOGI("%s: %f %f %f %f %f %f %f %f %f\n", __func__, mm[0], mm[1], mm[2], mm[3], mm[4], mm[5],
          mm[6], mm[7], mm[8]);
    return 1;
}
...
/*
 * This value is defined only for continuous mode and on-change sensors. It is the delay between two
 ...
 * FIFO may be full. maxDelay should always fit within a 32 bit signed integer. It is declared as 64
 * bit on 64 bit architectures only for binary compatibility reasons. Availability:
...
max_delay_t sensor_get_max_delay(int s) {
    ...
    if (min_supported_rate < rate_cap) min_supported_rate = rate_cap;

    /* return 0 for wrong values */
    if (min_supported_rate < 0.1) return 0;
...
int32_t sensor_get_min_delay(int s) {
    ...
    /* return 0 for wrong values */
    if (max_supported_rate < 0.1) return 0;
