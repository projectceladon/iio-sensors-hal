...
int sysfs_read(const char path[PATH_MAX], void *buf, int buf_len) {
    ...
    if (!path[0] || !buf || buf_len < 1) return -1;
...
static void str2int(const char *buf, void *v) { *(int *)v = atoi(buf); }

static void str2float(const char *buf, void *v) { *(float *)v = strtof(buf, NULL); }

static void str2uint64(const char *buf, void *v) { *(uint64_t *)v = atoll(buf); }
...
int64_t get_timestamp_realtime(void) { return get_timestamp(CLOCK_REALTIME); }

int64_t get_timestamp_boot(void) { return get_timestamp(CLOCK_BOOTTIME); }

int64_t get_timestamp_thread(void) { return get_timestamp(CLOCK_THREAD_CPUTIME_ID); }

int64_t get_timestamp_monotonic(void) { return get_timestamp(CLOCK_MONOTONIC); }
