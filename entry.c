...
static int activate(__attribute__((unused)) struct sensors_poll_device_t* dev, int handle,
                    int enabled) {
    int64_t entry_ts;
    int ret;
    int elapsed_ms;

    if (init_count == 0 || handle < 0 || handle >= sensor_count) return -EINVAL;
...
static int set_delay(__attribute__((unused)) struct sensors_poll_device_t* dev, int handle,
                     int64_t ns) {
    int i;
    ...
    for (i = 0; i < sensor[handle].base_count; i++) sensor_set_delay(sensor[handle].base[i], ns);

    return sensor_set_delay(handle, ns);
}

static int poll(__attribute__((unused)) struct sensors_poll_device_t* dev, sensors_event_t* data,
                int count) {
    if (init_count == 0 || !data || count < 1) return -EINVAL;

    return sensor_poll(data, count);
}

static int batch(__attribute__((unused)) struct sensors_poll_device_1* dev, int sensor_handle,
                 __attribute__((unused)) int flags, int64_t sampling_period_ns,
                 __attribute__((unused)) int64_t max_report_latency_ns) {
    return set_delay((struct sensors_poll_device_t*)dev, sensor_handle, sampling_period_ns);
}

static int flush(__attribute__((unused)) struct sensors_poll_device_1* dev, int handle) {
    return sensor_flush(handle);
}

static int close_module(__attribute__((unused)) hw_device_t* device) {
    if (init_count == 0) return -EINVAL;
...
static int initialize_module(const struct hw_module_t* module, const char* id,
    ...
    if (strcmp(id, SENSORS_HARDWARE_POLL)) return -EINVAL;
...
static struct hw_module_methods_t module_methods = {.open = initialize_module};
