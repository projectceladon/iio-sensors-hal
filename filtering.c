...
typedef struct {
    ...
    unsigned int history_size;    /* Number of recorded samples		      */
    unsigned int history_entries; /* How many of these are initialized	      */
    int history_index;            /* Index of sample to evict next time	      */
} filter_average_t;

static unsigned int partition(float* list, unsigned int left, unsigned int right,
...
static void denoise_median(sensor_info_t* info, sensors_event_t* data, unsigned int num_fields) {
    ...
    if (info->event_count == 1) denoise_median_reset(info);

    if (f_data->count < f_data->sample_size) f_data->count++;
...
static void denoise_average(sensor_info_t* si, sensors_event_t* data) {
    ...
    if (filter->history_size != history_size) {
        ...
        filter->history = (float*)realloc(
            filter->history, filter->history_size * filter->num_fields * sizeof(float));
        if (filter->history) {
            filter->history_sum =
    ...
    if (!filter->history || !filter->history_sum) return; /* Unlikely, but still... */
    ...
    for (f = 0; f < filter->num_fields; f++) {
        ...
        if (filter->history_entries == 0) return;
...
void setup_noise_filtering(int s) {
    ...
    if (sensor[s].quirks & QUIRK_NOISY) switch (sensor[s].type) {
            case SENSOR_TYPE_GYROSCOPE:
    ...
    if (cursor)
        ...
        if (cursor) sensor[s].filter_type = FILTER_TYPE_MOVING_AVERAGE;
    }
    ...
    if (cursor) {
        ...
        if (*cursor) window_size = atoi(cursor);
    }
...
/*
 * This is a circular buffer holding the last GLOBAL_HISTORY_SIZE events, covering the entire sensor
 * collection. It is intended as a way to correlate data coming from active sensors, no matter the
 * sensor type, over a recent window of time. The array is not sorted ; we simply evict the oldest
 * cell (by insertion time) and replace its contents. Timestamps don't necessarily grow
 * monotonically as they tell the data acquisition type, and that there can be a delay between
 * acquisition and insertion into this table.
 */
...
void record_sample(int s, const sensors_event_t* event) {
    ...
    if (sensor[s].report_pending == DATA_DUPLICATE) return;
