...
#define GYRO_CALIBRATION_PATH "/data/vendor/sensors/gyro.conf"
...
void gyro_cal_init(int s) {
    ...
    ret = fscanf(data_file, "%f %f %f %f", &version, &cal_data->bias_x, &cal_data->bias_y,
                 &cal_data->bias_z);
...
void gyro_store_data(int s) {
    ...
    ret = fprintf(data_file, "%f %f %f %f", GYRO_CAL_VERSION, cal_data->bias_x, cal_data->bias_y,
                  cal_data->bias_z);

    if (ret < 0) ALOGE("Gyro calibration - store data failed!");
...
static int gyro_collect(float x, float y, float z, gyro_cal_t* cal_data) {
    ...
    if (cal_data->count < GYRO_DS_SIZE) {
        if (x < cal_data->min_x) cal_data->min_x = x;

        if (y < cal_data->min_y) cal_data->min_y = y;

        if (z < cal_data->min_z) cal_data->min_z = z;

        if (x > cal_data->max_x) cal_data->max_x = x;

        if (y > cal_data->max_y) cal_data->max_y = y;

        if (z > cal_data->max_z) cal_data->max_z = z;
...
void calibrate_gyro(int s, sensors_event_t* event) {
    ...
    if (sensor[s].cal_level == 0)
        sensor[s].cal_level =
            gyro_collect(event->data[0], event->data[1], event->data[2], cal_data);
    ...
    if (sensor[s].cal_level) event->gyro.status = SENSOR_STATUS_ACCURACY_HIGH;
}
